...
def decode_detections(dets, info, calibs, cls_mean_size, threshold, decoupled):
    ...
    for i in range(dets.shape[0]):  # batch
        ...
        for j in range(dets.shape[1]):  # max_dets
            ...
            # print(">>>mushiyi>>>debug", dimensions[0], "scale_y", scale_y)
            # if dimensions[1]*dimensions[2]<2:
            #     dimensions[0] = h * depth / calibs[i].P2[0, 0]
            # print(">>>mushiyi>>>debug2", dimensions[0])
            if decoupled:
            ...
            y3d += (info['img_size_original'][i][1] - (info['img_size_croped'][i][1] + info['upper'][i])*scale_y)/2
...
def decode_detections_old(dets, info, calibs, cls_mean_size, threshold, decoupled):
    '''
    NOTE: THIS IS A NUMPY FUNCTION
    input: dets, numpy array, shape in [batch x max_dets x dim]
    input: img_info, dict, necessary information of input images
...
def extract_dets_from_outputs(outputs, K=50, topk=50):
    ...
    if "Ood_score" in outputs:
        ...
        prob = torch.cat([prob[:,:,0:-1], Ood_score], dim=2) # drop otherfg switch to Ood_score
    ...
    # print("boxes", boxes)
    # print("pred_sample_points", pred_sample_points)
    xs3d = boxes[:, :, 0: 1] 
    ys3d = boxes[:, :, 1: 2] 
