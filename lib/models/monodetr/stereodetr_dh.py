...
from .doublehead_transformer import build_double_head_transformer
...
class StereoDETRDH(nn.Module):
    """ This is the StereoDETR double head module that performs monocualr 3D object detection """
    ...
    def forward(self, images, calibs, targets, img_sizes, dn_args=None):
        ...
        pred_depth_map_logits, depth_pos_embed, weighted_depth,\
              depth_pos_embed_ip= self.depth_predictor(
            srcs_left, masks_left[1], pos_left[1])
        # 复制batch
        # pred_depth_map_logits = torch.cat([pred_depth_map_logits for _ in range(2)], 0)
        # depth_pos_embed = torch.cat([depth_pos_embed for _ in range(2)], 0)
        # depth_pos_embed_ip = torch.cat([depth_pos_embed_ip for _ in range(2)], 0)
        hs, init_reference, inter_references, inter_references_dim, \
            enc_outputs_class, enc_outputs_coord_unact = self.depthaware_transformer(
                srcs_stereo, masks_left, pos_left, query_embeds, depth_pos_embed, depth_pos_embed_ip)#, attn_mask)
...
class SetCriterion(nn.Module):
    ...
    def loss_disp_map(self, outputs, targets, indices, num_boxes):
        disp_map_logits = outputs['pred_disp']
        ...
        losses = dict()
        losses["loss_depth_map"] = self.disparity_loss(disp_map_logits, disp_maps)
        return losses
...
def build_stereo_dh(cfg):
    # backbone
    backbone = build_backbone(cfg)

    # detr
    depthaware_transformer = build_double_head_transformer(cfg)
