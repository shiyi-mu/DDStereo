...
class StereoDepthPredictor(nn.Module):
    ...
    def __init__(self, model_cfg):
        ...
        # self.cost_volume_0 = PSMCosineModule(downsample_scale=4, max_disp=96, input_features=256)
    ...
    def forward(self, feature_stereo, mask, pos):
        ...
        # src_32 = self.upsample(F.interpolate(feature_stereo[2], size=src_16.shape[-2:], mode='bilinear'))
        # src_8 = self.downsample(feature_stereo[0])
        # print(">>>>mushiyi>>> feature_stereo 0", feature_stereo[0].shape)
        # print(">>>>mushiyi>>> feature_stereo 1", feature_stereo[1].shape)
        # print(">>>>mushiyi>>> feature_stereo 2", feature_stereo[2].shape)
        # print(">>>>mushiyi>>> feature_stereo 3", feature_stereo[3].shape)
        # PSVolume_0 = self.cost_volume_0(feature_stereo[0][:batch_size_half], feature_stereo[0][batch_size_half:])
        ...
        # print(">>>>mushiyi>>> pred_disp", pred_disp.shape)
        # print(">>>>mushiyi>>> PSV_features_inter", PSV_features_inter.shape)
