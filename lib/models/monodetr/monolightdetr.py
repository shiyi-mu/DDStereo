...
class MonoLightDETR(nn.Module):
    """ This is the MonoDETR module that performs monocualr 3D object detection """
    def __init__(self, backbone, depthaware_transformer, num_classes, num_queries, num_feature_levels,
        ...
        print("*"*100, "self.Ood_det = ", self.Ood_det, type(self.Ood_det))
        print("self.Ood_det is not None = ", self.Ood_det is not None)
    ...
    def forward(self, images, calibs, targets, img_sizes, img_sizes_ori, img_sizes_upper, dn_args=None):
        ...
        left_images = images
        features, pos_stereo = self.backbone(left_images)
        ...
        if debug:
            pred_depth_map_logits = torch.ones([batch_size, 1, left_images.shape[-1], left_images.shape[-2]], device='cuda')
            weighted_depth_gt = self.sample_from_depth_map(pred_depth_map_logits, targets)
            weighted_depth = weighted_depth_gt
        ...
        for lvl in range(hs.shape[0]):
            ...
            if self.Ood_det is not None:
                outputs_class_fg = self.class_embed_fg[lvl](hs[lvl])
                outputs_classes_fg.append(outputs_class_fg)
        ...
        prob_normals = outputs_class[-1].sigmoid()
        ...
        if 'aux_outputs' in outputs:
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                for loss in self.losses:
                    ...
                    if loss in ['depth_map', 'labels_fg','dims','depths', 'angles', 'center', 'sample_point']:
...
def build_lightmono(cfg):
    ...
    if cfg['Ood_det'] is not None:
        losses = ['labels', 'labels_fg', 'boxes', 'cardinality', 'depths', 'dims', 'angles', 'center']
    else:
