...
class StereoLightDETROoD(nn.Module):
    ...
    def __init__(self, backbone, stereo_transformer, mono_transformer, depth_predictor, num_classes, num_queries, num_feature_levels,
                 ...
                 group_num=11, depth_pre_type="one", depth_sample_mode="3Dcenter",fusion_mode=None, num_scale=1, FPN_type=None,
                 Ood_det=None):
        ...
        self.class_embed_fg = nn.Linear(hidden_dim, 1)
    ...
    def sample_from_depth_map(self, depth_logits, targets):
        ...
        # target_3dcenter[:, :2] = torch.floor(target_3dcenter) # x1,y1
        if 0:
            target_3dcenter[:, :2] = torch.ceil(target_3dcenter) # x1,y1
            target_3dcenter = target_3dcenter.long()
            
            target_3dcenter = target_3dcenter.split(num_gt_per_img, dim=0)
            gt_center_depth = gt_center_depth.split(num_gt_per_img, dim=0)
            B = len(target_3dcenter)
            for b in range(B):
                print("b id = ", b)
                gt_boxes_per_batch = target_3dcenter[b]
                for n in range(gt_boxes_per_batch.shape[0]):
                    center_x, center_y = gt_boxes_per_batch[n]
                    print("debug center_x, center_y", center_x, center_y)
                    depth_map[b, center_y-1: center_y, center_x-1:center_x] = 1000
        return depth_map
    ...
    def forward(self, images, calibs, targets, img_sizes, img_sizes_ori, img_sizes_upper, dn_args=None):
        ...
        intermediate_output = self.stereo_transformer(srcs_left[1:4], masks_left[1:4], pos_left[1:4], query_embeds)
        ...
        for lvl in range(hs_stereo.shape[0]):
            ...
            # # classes
            # inter_class = self.class_embed[lvl](hs_stereo[lvl])
            ...
            # classes
            outputs_class = self.class_embed[lvl](hs_stereo[lvl])
            ...
            feature_for_depth_reg = hs_stereo[lvl]
            ...
            # angles
            outputs_angle = self.angle_embed[lvl](hs_stereo[lvl])
        ...
        if self.Ood_det is not None:
            ...
            out['pred_logits_fg'] = outputs_class_fg[-1]
...
class SetCriterion(nn.Module):
    ...
    def forward(self, outputs, targets, mask_dict=None):
        ...
        #     indices = self.matcher_stereo(inter_outputs, targets, group_num=group_num)
...
def build_lightstereoOod(cfg):
    ...
    if cfg['Ood_det'] is not None:
        weight_dict['loss_fg'] = cfg['cls_loss_coef_fg']
