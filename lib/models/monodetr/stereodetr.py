...
from .depth_predictor import StereoDepthPredictor
...
class StereoDETR(nn.Module):
    ...
    def forward(self, images, calibs, targets, img_sizes, dn_args=None):
        ...
        pred_depth_map_logits, depth_pos_embed, weighted_depth,\
            ...
            srcs_stereo, masks_left[1], pos_left[1])
        srcs_left[1] = features_for_depth
        srcs_left[0] = srcs_left[0] * 1.
        srcs_left[2] = srcs_left[2] * 1.
        srcs_left[3] = srcs_left[3] * 1.
        # masks_left[-1] = masks_left[1]
        # pos_left[-1] = pos_left[1]
        # srcs_left.append(features_for_depth)
        # masks_left.append(masks_left[1])
        # pos_left.append(pos_left[1])
        hs, init_reference, inter_references, inter_references_dim, \
            enc_outputs_class, enc_outputs_coord_unact = self.depthaware_transformer(
                srcs_left, masks_left, pos_left, query_embeds, depth_pos_embed, depth_pos_embed_ip)#, attn_mask)
...
def build_stereo(cfg):
    ...
    depth_predictor = StereoDepthPredictor(cfg)
