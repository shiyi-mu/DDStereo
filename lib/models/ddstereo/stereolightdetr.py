...
from .matcherstable import build_Stablematcher
...
class StereoLightDETR(nn.Module):
    def __init__(self, backbone, depthaware_transformer, depth_predictor, num_classes, num_queries, num_feature_levels,
                 aux_loss=True, with_box_refine=False, with_center_refine=False,two_stage=False, init_box=False, use_dab=False, group_num=11, two_stage_dino=False, depth_pre_type="one", fusion_mode=None):
    ...
    def forward(self, images, calibs, targets, img_sizes, dn_args=None):
        ...
        pred_depth_map_logits, depth_pos_embed, weighted_depth,\
              depth_pos_embed_ip, features_for_depth, pred_disp = self.depth_predictor(
            srcs_stereo, masks_left[2], pos_left[2], targets)
        if self.fusion_mode == "concat":
            # srcs_left[0] = self.fuse_conv(torch.concat([srcs_left[0] , features_for_depth], dim=1))     
            # features_for_depth_1 = self.pooling(features_for_depth)
            srcs_left[2] = self.fuse_conv1(torch.concat([srcs_left[2] , features_for_depth], dim=1)) 
            # features_for_depth_2 = self.pooling(features_for_depth_1)
            # srcs_left[2] = self.fuse_conv2(torch.concat([srcs_left[2] , features_for_depth_2], dim=1)) 
        ...
        for lvl in range(hs.shape[0]):
            ...
            if self.with_center_refine:
                center_refine_offset = self.center_refine_offset[lvl](hs[lvl]).unsqueeze(2)
                outputs_center3d = outputs_center3d + center_refine_offset
            ...
            # save weighted_depth to rgb image
            if 0:
                weighted_depth_image = weighted_depth.unsqueeze(1)[0].detach().cpu().numpy()
                # 归一化
                weighted_depth_image = (weighted_depth_image - weighted_depth_image.min()) \
                                        / (weighted_depth_image.max() - weighted_depth_image.min())
                weighted_depth_image = np.transpose(weighted_depth_image, (1, 2, 0))*255
                cv2.imwrite("./weighted_depth.png", weighted_depth_image)
            # depth average + sigma
            if self.depth_pre_type == "avg":
                ...
                depth_ave = torch.cat([(depth_reg[:, :, 0: 1]),
                                       depth_reg[:, :, 1: 2]*0], -1)
            elif self.depth_pre_type == "one_w_uncertainty":
                depth_ave = torch.cat([depth_map,
                                    depth_reg[:, :, 1: 2]], -1)
...
class SetCriterion(nn.Module):
    """ This class computes the loss for DDStereo.
    The process happens in two steps:
        1) we compute hungarian assignment between ground truth boxes and the outputs of the model
        2) we supervise each pair of matched ground-truth / prediction (supervise class and box)
    """
    def __init__(self, num_classes, matcher, weight_dict, focal_alpha, losses, group_num=11, depth_sort_reverse=False):
        ...
        self.ddn_loss = DDNLoss(depth_sort_reverse=depth_sort_reverse)  # for depth map
...
def build_lightstereo(cfg):
    ...
    # matcher
    matcher = build_Stablematcher(cfg)
