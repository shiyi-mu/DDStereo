...
class StableHungarianMatcher(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, outputs, targets, group_num=11, match_mode="mono"):
        ...
        elif match_mode == "stereo":
            out_prob = outputs["pred_logits_fg"].flatten(0, 1).sigmoid()  # [batch_size * num_queries, 2]
        else:
