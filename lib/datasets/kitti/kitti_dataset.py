...
class KITTI_Dataset(data.Dataset):
    def __init__(self, split, cfg, 
                 subset_train="ori_train",
                 subset_val="ori_val"):
        # basic configuration
        self.image_type = "png"
        if subset_train == "ori_train":
            self.root_dir = cfg.get('root_dir_ori_train')
            self.train_txt = cfg.get('train_txt_ori_train')
        elif subset_train == "ar_ind":
            self.root_dir = cfg.get('root_dir_ar_ind')
            self.train_txt = cfg.get('train_txt_ar_ind')
        elif subset_train == "vkitti_clone":
            self.root_dir = cfg.get('root_dir_vkitti_clone')
            self.train_txt = cfg.get('train_txt_vkitti_clone')
            self.image_type = "jpg"
        else:
            raise ValueError("unknown subset_train") 
        # basic configuration
        # self.root_dir = cfg.get('root_dir')
        # self.train_txt = cfg.get('train_txt', None)
        if subset_val == "ori_val":
            self.root_dir_eval= cfg.get('root_dir_ori_val')
            self.eval_txt = cfg.get('eval_txt_ori_val', None) 
        elif subset_val == "ood_val":     
            self.root_dir_eval= cfg.get('root_dir_ood_val')
            self.eval_txt = cfg.get('eval_txt_ood_val', None) 
        else:
            raise ValueError("unknown subset_val") 
        ...
        self.writelist_OtherForeground = cfg.get('writelist_OtherForeground')
        self.writelist_test_Ood = cfg.get('writelist_test_Ood')
        self.cls2id = {}
        for i, cls in enumerate(self.writelist_train):
            self.cls2id[cls] = i
        # self.cls2id["Ood"] = len(self.writelist_train)
        # self.resolution_ori = np.array([1280, 388])  # W * H
        ...
        print(">>>>>mushiyi>>>> self.drop_unkown_level", self.drop_unkown_level)
        print(">>>>>mushiyi>>>> self.get_depth_sample_points", self.get_depth_sample_points)
        print(">>>>>mushiyi>>>> self.get_disparity_on_the_fly", self.get_disparity_on_the_fly)
        ...
        print(">>>self.eval_txt >> ", self.eval_txt)
        ...
        print(">>>>>mushiyi>>>> self.data_augmentation", self.data_augmentation)
        ...
        print(">>>>>mushiyi>>>> self.stereo_shift_aug", self.stereo_shift_aug)
    ...
    def get_image(self, idx):
        filename = f'{idx}.' + self.image_type
    ...
    def get_image3(self, idx):
        ...
        if self.image_type == "jpg":
            filename = filename.replace("Camera-0.jpg", "Camera-1.jpg")
        img3_file = os.path.join(self.image3_dir, filename)
        assert os.path.exists(img3_file), img3_file
        return Image.open(img3_file)    # (H, W, 3) RGB mode
    
    def get_disparity_P2(self, idx):
        filename = f'P2{idx}.png'
        disp_file = os.path.join(self.disparity_dir, filename)
        if not os.path.exists(disp_file):
            print("xxxxx>>>>>>>>>>>", disp_file)
            return None
    ...
    def get_calib(self, idx):
        filename = f'{idx}.txt'
        calib_file = os.path.join(self.calib_dir, filename)
        assert os.path.exists(calib_file)
        return Calibration(calib_file)
    
    def new_class2ood(self, gt_annos):
        for anno in gt_annos:
            for index in range(len(anno["name"])):
                if anno["name"][index] in self.writelist_test_Ood:
                    anno["name"][index] = 'OoD'
        return gt_annos
    ...
    def eval(self, results_dir, logger):
        ...
        dt_annos =  self.new_class2ood(dt_annos)
        
        gt_annos = kitti.get_label_annos(self.label_dir, img_ids)
        gt_annos = self.new_class2ood(gt_annos)
    ...
    def __getitem__(self, item):
        ...
        # print(">>>DEBUG DATSET mushiyi>>> read img.size >>>>> ", img.size)
        # import cv2
        # debug_img = cv2.cvtColor(np.asarray(img),cv2.COLOR_RGB2BGR)
        # cv2.imwrite("debug_img_ORI.png",  debug_img)
        ...
        # print("DEBUG transform >>> img.shape:", img.shape)
        if self.crop_top > 0:
        ...
        # print("DEBUG croptop_func>>> img.shape:", img.shape)
        # # SAVE DEBYG IMAGE AND DISP
        # import cv2
        # debug_img = ((img.transpose(1,2,0) * self.std + self.mean)*255).astype(np.uint8)
        ...
        labels = np.ones((self.max_objs), dtype=np.int8)*(-1)
        ...
        for i in range(object_num):
            if self.ori_size_check_filter == True:
                if img_size_original[0] != 1242:
                    # print("Wrong image size", img_size_original)
                    continue
            # filter objects by writelist
            if objects[i].cls_type in self.writelist_OtherForeground:
                objects[i].cls_type = "OoD"
            elif objects[i].cls_type in self.writelist_test_Ood:
                objects[i].cls_type = "OoD"
        ...
        targets = {
                   ...
                   'img_id': str(index)}
        #print(">>>debug>>> targets/.keys(): ", targets.keys(), flush=True)
        info = {'img_id': str(index),
                'img_size_original': img_size_original,
                'img_size_croped': img_size_croped,
                "upper": self.croptop_func.upper,
                'bbox_downsample_ratio': img_size_croped / features_size}
